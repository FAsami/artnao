  ...
  return (
    <header className="h-20  w-screen z-50 bg-white relative">
      <div className="max-w-screen-xl px-3 mx-auto flex items-center justify-between h-full">
        ...
        <div className="flex items-center gap-6 text-base font-semibold text-gray-700 tracking-wider relative px-4 h-full">
          {navigation.map(({ id, title, url }) => {
            return (
              <Link
                key={id}
                className={clsx(
                  'cursor-pointer  relative z-10 h-full flex items-center',
                  path === url && 'border-b-4 border-b-amber-400 px-2'
                )}
                href={url}
              >
                <span>{title}</span>
